...
const events = [
	...
	},
	{
		...
		overview:
			'DevOps Unleashed is a hands-on conference featuring workshops and talks on modern DevOps practices, including Kubernetes, Docker, GigOps, and infrastructure as code.',
		image: '/images/event1.png',
		venue: 'Navy Pier',
		location: 'Chicago, IL',
...
async function bootstrap() {
	...
	try {
		...
		const organizer = await userModel.create({
			fullName: 'Aditya Tripatahi',
			username: 'adityatripathi',
			email: 'me@adityatripathi.dev',
			password: '1234567890',
			roles: [Role.ORGANIZER, Role.ADMIN, Role.USER],
		});
		...
		const imagesPath = path.join(process.cwd(), '../frontend/public/images');
		...
		for (const eventData of events) {
			try {
				...
				console.error(`Failed to seed event ${eventData.title}:`, error);
			}
		}
