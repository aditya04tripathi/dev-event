...
export class BookingService {
	...
	private async generateQRCode(booking: BookingDocument, event: EventDocument) {
	...
	async scanTicket(eventIdOrSlug: string, checkInDto: CheckInDto) {
		...
		const booking = await this.bookingModel.findById(
			new mongoose.Types.ObjectId(decryptedData.bookingId),
		);
		if (!booking) {
			throw new NotFoundException('Booking not found');
		}

		if (!booking.event.equals(event._id)) {
	...
	async getEventParticipants(
		...
		const participants = await this.bookingModel
			...
			.populate({
				path: 'event',
				select:
					'title date location slug image description venue time mode audience organizer tags overview agenda',
				populate: {
					path: 'organizer',
		...
		const result = participants.map((participant: any) => ({
			...participant,
		...
		return {
			...
			skip: skip,
			nextPage:
				pageNum < Math.ceil(totalParticipants / limitNum) ? pageNum + 1 : null,
